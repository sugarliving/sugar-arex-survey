<script>
(async () => {
  const categories = await $.getJSON("https://raw.githubusercontent.com/sugarliving/sugar-arex-survey/main/categories.json");
  const questions = await $.getJSON("https://raw.githubusercontent.com/sugarliving/sugar-arex-survey/main/questions.json");
	
  const icons = {
    unchecked: "https://uploads-ssl.webflow.com/5f308a2a4e2694784fd8c994/61560ef446f49f558702d0a0_icon_survey_answer_unchecked.svg",
    checked: "https://uploads-ssl.webflow.com/5f308a2a4e2694784fd8c994/61560ef4e7afa55aef88e0a8_icon_survey_answer_checked.svg",
  };
  
  const state = {
    active_question_index: 0,
    active_question: questions[0],
    read_intro: JSON.parse(localStorage.getItem("sugar_arex_scorecard_read_intro"))
  };
  
  // Display the intro if they haven't readit yet
  const setIntroModal = function() {
  	if (!state.read_intro || state.read_intro === "false") {
  ...
  $("#survey-intro-continue-button").on("click", function(e){
  	e.preventDefault();
  	localStorage.setItem("sugar_arex_scorecard_read_intro", "true");
    state.read_intro = true;
    setIntroModal();
  });
  ...
  const setFinalScorecard = function () {
    ...
    $("#wf-form-AREX-Survey-Form input[id*=score]").each(function () {
      finalScorecard.forEach((category) => {
        if (category.hasOwnProperty(categories[$(this).attr("category")].name)) {
          category[categories[$(this).attr("category")].name].scores.push(parseInt($(this).val()));
          category[categories[$(this).attr("category")].name].max_scores.push(parseInt($(this).attr("maxscore")));
        }
      });
